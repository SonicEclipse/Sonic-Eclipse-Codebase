var cur_speed;
function player_state_fast_spindash(){
	//Trigger the fast-spindash
	if(press_c && character == CHAR_SONIC && ground
	&& !force_roll && (ground_angle < 40 || ground_angle > 320))
	{
		...
		//Save current speed
		cur_speed = ground_speed;
		
		//Update the state
		state = ST_FAST_SPINDASH;
	}
	
	//Stop executing if not fast-spindashing
	if(state != ST_FAST_SPINDASH)
		exit;
	...
	if(!hold_c)
	{
		...
		if(ground)
		{
			ground_speed = (8+(floor(spindash_rev)/2)+(cur_speed/12)) * facing;
		}
		else
		{
			x_speed = (8+(floor(spindash_rev)/2)+(cur_speed/12)) * facing;
		}
		state = ST_ROLL;
	}
}